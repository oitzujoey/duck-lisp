...
static dl_error_t ast_generate_constant(duckLisp_t *duckLisp, duckLisp_ast_constant_t *constant,
	...
	switch (constantCST.type) {
	case cst_constant_type_number:
		constant->type = ast_constant_type_number;
		e = ast_generate_number(duckLisp, &constant->value.number, constantCST.value.number, throwErrors);
		break;
	case cst_constant_type_string:
		constant->type = ast_constant_type_string;
...
static dl_error_t ast_generate_compoundExpression(duckLisp_t *duckLisp, duckLisp_ast_compoundExpression_t *compoundExpression,
	...
	case cst_compoundExpression_type_constant:
		...
		break;
	case cst_compoundExpression_type_identifier:
		compoundExpression->type = ast_compoundExpression_type_identifier;
		...
		break;
	case cst_compoundExpression_type_expression:
		compoundExpression->type = ast_compoundExpression_type_expression;
...
dl_error_t duckLisp_generator_label(duckLisp_t *duckLisp, dl_array_t *assembly, duckLisp_ast_expression_t *expression) {
	...
	e = duckLisp_emit_label(duckLisp, assembly, expression->compoundExpressions[1].value.constant.value.string.value,
...
dl_error_t duckLisp_generator_goto(duckLisp_t *duckLisp, dl_array_t *assembly, duckLisp_ast_expression_t *expression) {
	...
	e = duckLisp_emit_jump(duckLisp, assembly, expression->compoundExpressions[1].value.constant.value.string.value,
	                       expression->compoundExpressions[1].value.constant.value.string.value_length);
	if (e) {
		goto l_cleanup;
	}
